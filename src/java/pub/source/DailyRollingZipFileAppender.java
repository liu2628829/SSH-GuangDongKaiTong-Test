...
/**
 * 每天自动压缩日志文件。
 * 增加删除文件， 当压缩文件超过 （默认7天） 就删除。
 * sxming modify in 2007-11-1 18:40:00 
 * Similar to <code>org.apche.log4j.DailyRollingFileAppender</code>,
 * DailyRollingZipFileAppender extends {@link FileAppender} so that the
 * underlying file is rolled over at a user chosen frequency. <B>Further more,
 * DailyRollingZipFileAppender will zip the backup log file to save disk
 * space.</B>
 * 
 * <p>
...
public class DailyRollingZipFileAppender extends FileAppender {
	...
	/**
	 * fileReservePeriod 文件保留周期 默认为 7 sxming add
	 */
	private int fileReservePeriod = 7;
	...
	public DailyRollingZipFileAppender(Layout layout, String filename,
			String datePattern, String fileReservePeriod) throws IOException {
	...
	void rollOver() throws IOException {
		...
		// 到周期后将 文件删除 sxming add
		java.util.GregorianCalendar gc = new java.util.GregorianCalendar();
		gc.setTime(now);
		gc.add(Calendar.DATE, -1 * this.fileReservePeriod);
		String delfilename = fileName + sdf.format(gc.getTime()) + ".zip";
		// System.err.println("delfilename:"+delfilename);
		...
		if (result) {
			...
			if (result) {
				...
				LogLog.error("Failed to zip [" + fileName + "] to ["
						+ scheduledFilename + ".gzip].");
			}
	 ...
	 * @param name
	 *            String name of the newly created zip file
	 */
	static boolean zip(File file, String name) {
		try {
			...
			zout.close(); // !!!关闭流,必须关闭所有输入输出流.保证输入输出完整和释放系统资源.
			fout.close();
			fin.close();

		} catch (IOException e) {
			LogLog.error(
					"catch IOException while zipping file " + file.getName(), e);
			return false;
		}
		return true;
	}
