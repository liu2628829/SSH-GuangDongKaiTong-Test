...
if(_59.showPageList){
var ps="每页<select class=\"pagination-page-list\">";
...
html+="<td><input title=\"输入后按enter键\" class=\"pagination-num\" type=\"text\" value=\"1\" size=\"9\"></td>";
...
var _69=Math.ceil(_68.total/_68.pageSize)||0;//||1; the default pageSize is 0
var num=$(_67).find("input.pagination-num");
num.val(_68.total==0?0:_68.pageNumber);//num.val(_68.pageNumber);//when no data，the defalut pageNumber is 0
num.parent().next().find("span").html(_68.afterPageText.replace(/{pages}/,_69));
var _6a=_68.displayMsg;
_6a=_6a.replace(/{from}/,_68.pageSize*(_68.pageNumber-1)+(_68.total==0?0:1));//+1);//when no data, is 0
...
if(_72){
//_6f.total= _72["options"].pageSize;//加上这句即可控制永远只显示第1页
...
var _173=$("<div class=\"panel-header\"><div class=\"panel-title\" title='"+(opts.tipTitle?opts.tipTitle:"")+"'>"+opts.title+"</div></div>").prependTo(_172);
if(opts.iconCls){
...
$(_1c5).window("move");
if(_1c6.proxy){ //解决jquery弹窗在IE8、7快速拖动js偶尔报错问题
	_1c6.proxy.remove();
	_1c6.proxy=null;
}
}});
...
_1bb(this);
_1c4(this);
var _172 = $.data(this,"panel").panel;
$.data(this,"panel").panel.append("<iframe frameborder='0' style='position:absolute;left:0px;top:0px;height:"+_172.height()+"px;width:"+_172.width()+"px;filter:Alpha(opacity=0);z-index:-1;'></iframe>");
});
};
...
var win = null;
if(opts && opts.winId){
	win = $("<div class=\"messager-body\" id=\""+opts.winId+"\"></div>").appendTo("body");
}else{
	win = $("<div class=\"messager-body\"></div>").appendTo("body");
}
win.append(_1ef);
if(_1f0){
...
//upd by zwb 增加closable:false属性
win.window({title:_1ee,noheader:(_1ee?false:true),width:opt.width,height:"auto",modal:true,collapsible:false,closable:false,minimizable:false,maximizable:false,resizable:false,onClose:function(){
setTimeout(function(){
win.window("destroy");
},100);
}});
...
//定时自动关闭窗口 tangyj 2013-2-22 
if(opts && opts.timeout){
	if(opts.timeout != '0' ){
		setTimeout(function(){try{win.window("close");}catch(e){}},opts.timeout);
	}
}
...
pp.panel("body").css("overflow","auto"); //换了jquery 1.6后，accrdion的滚动条会自动变成隐藏的，加上这句就好了
return false;
});
};
...
var pp=$(this);
var rightNo = pp[0].rightNo;
tabs.push(pp);
_260(_257,pp);
$.fn.pushTabs(tabs,cc,rightNo,pp)//edit by pjw
});
...
$.fn.pushTabs = function(tabs,cc,rightNo,pp){//a by pjw
};
...
var td=$("<td "+attr+" "+(col.title?("title='"+col.title+"'"):"")+" ></td>").appendTo(tr); //modify by qiaoqd. title属性要加单引号,否则设计器的属性区datagrid表头列名会错乱
if(col.checkbox){
...
$("<div class=\"datagrid-header-check\"></div>").html((col.showTitleCheck+"")!="false"?("<input type=\"checkbox\" " + str + " />"):"&nbsp;&nbsp;&nbsp;&nbsp;").appendTo(td);
}else{
if(col.field){
td.attr("field",col.field);
...
var rows = body.find("tr[datagrid-row-index]");
if(opts.rowOverColorEnable){//add by gt 2013/06/07  the if condition
	rows.unbind(".datagrid").bind("mouseenter.datagrid",function(){
...
if(opts.onClickRow){
	rows.bind("click.datagrid",function(){
...
if(opts.onDblClickRow){
	rows.bind("dblclick.datagrid",function(){
...
if(opts.onRowContextMenu){
	rows.bind("contextmenu.datagrid",function(e){
...
var tds = body.find("td[field]");
if(opts.onClickCell){
	tds.unbind(".datagrid").bind("click.datagrid",function(){
...
if(opts.onDblClickCell){
	tds.bind("dblclick.datagrid",function(){
...
//表头排序时，如果当前列表有分页，并且能拿到分页栏对象，则重置分页栏 //a by gt
if(opts.pagination && data && data.rows && data.rows.length > 0 && (data.rows[0].iRecCount || data.rows[0].totalCount)){
	var pageNumber=1;
	if(opts.pagination){
		var p =$.data(_40c,"datagrid").panel.find("div.datagrid-pager");
		var pager = $(p).pagination("options");
		pageNumber=pager.pageNumber;
	}
	var total = data.rows[0].iRecCount ? data.rows[0].iRecCount : data.rows[0].totalCount;
	var json ={total:total, pageNumber:pageNumber};
	$('#'+_40c.id).datagrid('getPager').pagination(json); 
}
...
var _4d2=$("<input type=\"checkbox\" onclick='var obj = this; setTimeout(function(){obj.checked=!obj.checked},5)'>").appendTo(_4d0);
_4d2.val(_4d1.on);
...
_43d(this,data);
_4a2(this);
//add by gt 数据重新加载 去掉全选选中
var _455=$.data(this,"datagrid").panel;
var headCK=_455.find("div.datagrid-header-check input[type=checkbox]"); 
var flg = $.fn.prop ? (headCK.prop("checked",false)) : (headCK.attr("checked",false));
});
},getData:function(jq){
...
var title = (col.displayTip)?("title=\""+((_53a[_53c]!=null&&_53a[_53c]!=undefined)?_53a[_53c]:"")+"\" "):""; //add by gt
cc.push("<div style=\""+_53e+"\" "+title);
if(col.checkbox){
...
//add by gt 更新一行，也要重置tip
if((col.displayTip+"").toLowerCase()=="true"){
  cell.attr("title",rows[_543][_545]);
}
}
}
...
//a by gt 默认选中第一行
if(typeof(_5ca)!="undefined"&&_5ca!=null){ //分层加载，则选被加载的节点
_5f6(_5c9,_5ca);
}else{//否则加载第一个节点
var roots=_5de(_5c9);if(roots&&roots.length>0)var id=roots[0][opts.idField];_5f6(_5c9,id);
}
/**
treegrid，初始就有子节点数据时，n个子节点所归属的上层tr都会有一个高度值，一旦有此高度值，收缩时，就留下一片空白
因为此高度时有时无，很奇怪，找不到什么地方有给设置高度，所以暂用以下语句，进行清理  add by gt 
*/
function cleanH(){$(_5cd).find("*.treegrid-tr-tree").height(0);}
window.setTimeout(cleanH, 100);
...
if(!opts.url){
if(_5d9)_5d9();//add by qiaoqide 因url为空，树形列表节点第一次展开没有调用展开事件。
return;
}
...
if(!opts.nowrap){return;} //单元格内容撑开，将无法确定每一行的高度，所以不进行可视定位

/**/var _45b=$.data(_5f7,"datagrid").panel;
var _45d=_45b.find("div.datagrid-view2");
var _45f=_45d.find("div.datagrid-body");
var allTrs = $(_45f).find("tr[node-id]");//所有行
var hideTr2 = $(_45f).find("div:hidden").find("tr[node-id]"); //隐藏的行数
var showTrs=[];
for(var i=0;i<allTrs.length;i++){
	var r = allTrs[i];
	var flg = true;
	for(var j=0;j<hideTr2.length;j++){
		if($(r).attr("node-id") == $(hideTr2[j]).attr("node-id")){
			flg = false; break;
		}
	}
	if(flg){showTrs.push(r);}
}
var ct=0;
for(var i=0;i<showTrs.length;i++){
	var r = showTrs[i];
	if($(r).attr("node-id") == $(tr).attr("node-id") ){
		ct = i+1;break;
	}
}
var tds = $(tr).find("td"); //var trh = $(tr).find("td").outerHeight(); 用outerHeight方法，影响了树型第一行的呈现，背景色透视在文字之上，原因未明
var td = null;
for(var i=0;i<tds.length;i++){
	var tptd = $(tds[i]).get(0) ;
	if(tptd.style.display!="none"){td = tptd;break;}
}
var trh = (td == null) ? 24 : td.clientHeight;
var h1= $(_45f).height();
var h = ct*trh; //行所在高度位置，定位到列表中央
var curScorllTop = _45f.scrollTop()+h1; //最下位
var curScorllTop2 = _45f.scrollTop(); //最上位
if(!(h>=(curScorllTop2+trh) && h<=curScorllTop)){//不在可视区，才滚动
  if(h>curScorllTop){
  	_45f.scrollTop(h-h1*0.5);
  }else{
  	_45f.scrollTop(h-trh);
  }
}
};
...
var title = (col.displayTip)?("title=\""+((row[_63e]!=null&&row[_63e]!=undefined)?row[_63e]:"")+"\" "):"";//add by gt
cc.push("<div style=\""+_640+"\" "+title);
if(col.checkbox){
cc.push("class=\"datagrid-cell-check ");
}else{
cc.push("class=\"datagrid-cell ");
...
$.fn.datagrid.defaults.view.renderRow = function(target, fields, frozen, rowIndex, rowData){
	...
	for(var i=0; i<fields.length; i++){
		...
		if (col){
			...
			var title = (col.displayTip)? ("title=\""+((rowData[field]!=null&&rowData[field]!=undefined)?rowData[field]:"")+"\" "):""; //add by djf
...
$.extend($.fn.datagrid.defaults.editors, {
		...
		moreValue: {
			init: function(container, options){
				...
				textarea.id="tid";
				options.mId = textarea;
				var textarea1=$(textarea).attr("style","height:21px;overflow:hidden").appendTo(container);
				commonUtil.changeMoreValue(options);
				//因为上一行内部克隆了一个textarea,所以这里得重取一次
				textarea1= $(container).find("textarea")[0]; 
				return textarea1; //实际将被用于其它方法的target参数
			},
			getValue: function(target){
				return $(target).val();
			},
			...
			resize: function(target, width){
				var input = $(target);
				var w = $.browser.mozilla ? 14 : 15; //firefox边框占了一个px宽度
				if ($.boxModel == true){
					input.width(width - w - (input.outerWidth() - input.width()));
				} else {
					input.width(width - w);
				}
			}
		...
		number: {
			init: function(container, options){
				var input = $('<input style="width:120px;height:21px;">').appendTo(container);
				input1= input.get(0); 
				input1.id="numberId";
				options.eId=input1;
				commonUtil.initDigit(options);
				input1= $(container).find("input")[0]; 
				return input1;
			},
			getValue: function(target){
				return $(target).val();
			...
			resize: function(target, width){
				...
				var w = $.browser.mozilla ? 14 : 15; //firefox边框占了一个px宽度
				if ($.boxModel == true){
					w = width - w - (input.outerWidth() - input.width());
				} else {
					w = width - w;
				}
				input.css({width:w});
		...
		datePicker: {
			init: function(container, options){
				var input = $('<input class="Wdate" style="width:120px;height:21px;">').appendTo(container);
				input.bind("focus",function(){WdatePicker(options);});//onfocus="WdatePicker({dateFmt:'yyyy年M月d日',vel:'d244_2'})"
				return input;
			},
			getValue: function(target){
				return $(target).val();
			...
			resize: function(target, width){
				...
				var w;
				if ($.boxModel == true){
					w = width - (input.outerWidth() - input.width());
				} else {
					w = width;
				}
				input.css({width:w});
		...
		popupWindow: {
			init: function(container, options){
				var input = $('<input class="PopupWindow" style="width:120px;height:21px;">').appendTo(container);
				input.bind("focus",function(){WdatePicker(options);});//onfocus="WdatePicker({dateFmt:'yyyy年M月d日',vel:'d244_2'})"
				return input;
			},
			getValue: function(target){
		...
		select: {
			init: function(container, options){
				var input = $('<select name="btns" style="width:120px;height:21px;"></select>').appendTo(container);
				if(options.onchange){input.bind("change",options.onchange);}
				var opts = "<option value=''>请选择</option>";
				if(options.data){
					var objs = options.data;
				    for(var i =0;i<objs.length;i++){
				       opts += "<option value='" + objs[i][options.valueField] + "'>" + objs[i][options.textField] + "</option>";
				    }
				}
				$(input).html(opts);
				return input;
