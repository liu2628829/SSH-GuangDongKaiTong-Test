	    	...
	    	*/
            $.ajax({
                ...
                success: function(data, textStatus) {//第一次发起请求成功
                    ...
                    //simpleAlert({msg:'请点击确定开始下载！',type:2,OK:"下载",fn:function(){
                    if(data.indexOf("\"SUCCESS\":\"false\"")>0){
                    	simpleAlert("模板生成失败，请重试！");
                    	return;
                    }
        ...
        createInfo:function(data,fun){
            ...
            switch(msg.iState){
                case "-1"://当前导入线程超标
                	...
                	if(msg.SAME_IMPORT!=null){
                		simpleAlert("当前有其他用户在对【"+msg.sTemplateName+"】表进行数据导入，请稍后操作！");
                	}else{
                    	simpleAlert("系统最多允许同时存在"+msg.MAX_IMPORT+"个导入进程，目前"+msg.MAX_IMPORT+"个进程，正被占用中，请稍后操作！");
                    }
                    break;
                case "0" ://进行中
                    if(!msg.totalCount){
            ...
            case "3"://发现有不合格的数据,由用户确认是否继续
                if(msg.totalCount == cts[2]){ //不合格记录数与总记录行数一样多，直接提示下载错误文件
	                ...
	                simpleAlert({
	                    msg:"总计"+msg.totalCount+"行数据中，发现"+cts[2]+"行不合格，确定继续导入吗？", 
	                    type:2, 
	                    icon:"question", 
	                    fn:function(){//继续
	                        if(typeof(WaitBar) != "undefined"){WaitBar.show(2);}
		    ...
		    case "7":
		        Catt_AjaxImprotFile.setWaitBar();//关掉waitbar
		        simpleAlert("数据导入失败，导入过程中，系统发生未知异常！请联系系统厂家协助排查！");
		        break;
		    case "8":
		        Catt_AjaxImprotFile.setWaitBar();//关掉waitbar
		        if(msg.IMP_MAX_FILESIZE!=null){
		            simpleAlert("所上传的数据文件太大，不应超过："+msg.IMP_MAX_FILESIZE+" KB");
		        }else{
		            simpleAlert("所传文件不合规范: "+msg.msg);
		        }
		        break;
		    case "9":
		    ...
		    case "12":
		    	if(msg.IMP_MAX_LINES != null){ //CURRENT_MAX_LINES
			    	//simpleAlert("文件数据条数不允许超过" + msg.IMP_MAX_LINES + "条");
			    	var l = msg.CURRENT_MAX_LINES - msg.IMP_MAX_LINES ;
			    	simpleAlert("文件数据行数越限，最大允许"+ msg.IMP_MAX_LINES +"行，当前实际"+msg.CURRENT_MAX_LINES+"行，超出"+l+"行");
			    	Catt_AjaxImprotFile.setWaitBar();
		    	}
		    	break;
		    case "13":
		    	...
		    	if(msg.isFileClosed == "1"){
			    	...
			    	simpleAlert({
			    		msg:"错误单元格过多，目前已经发现"+msg.errCommentCount+"个。程序如果继续运行将极有可能导致错误批注信息无法回写至文件，甚至导致内存溢出、系统崩溃的风险！\n所以请先下载文件，按文件内的错误批注信息解决好已分析出的错误后再进行导入，谢谢！",
			    		opts:{width:500},
			    		fn:function(){
			    			if(typeof(WaitBar) != "undefined"){
			                	WaitBar.show(2);
...
createFileUploadpage:function(){
    ...
    if(!div){
        ...
        "<form method='post'>"+
        "<table width='100%' height='50px' class='formbasic'><tr><td width='25%'>选择数据文件</td>"+
